...
def write_manifest(
    ...
    *,
    ...
    manifest = {
        ...
        "geoid_model": "EGM2008",
        "geoid_provider": "GeographicLib/PROJ",
        "floor_height_m": floor_height_m,
        "terrain_tile_scheme": terrain_tile_scheme,
        "building_partition_scheme": building_partition_scheme,
...
class GlobalWGS84Ellipsoid3DMap:
    ...
    def query_buildings(self, lon: float, lat: float) -> Any:
        ...
        if self.building_paths:
            return self._query_buildings_duckdb(lon, lat)
    ...
    def _query_buildings_duckdb(self, lon: float, lat: float) -> Any:
        gpd = _require_geopandas()
        try:
            import duckdb
            ...
            raise ImportError(
                "duckdb and shapely are required for scalable building queries"
            ) from exc

        paths_expr = _duckdb_path_list(self.building_paths)
        query = f"""
            SELECT
                id,
            ...
            WHERE ST_Covers(geometry, ST_Point({float(lon)}, {float(lat)}))
        """
        ...
        frame["geometry"] = frame["geometry"].map(
            lambda value: wkb.loads(bytes(value) if isinstance(value, bytearray) else value)
        )
